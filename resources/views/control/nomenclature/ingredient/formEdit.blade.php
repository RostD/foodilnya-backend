...
@section('title','Редактирование ингредиента')

@section('content')

    <form id="formProp" style="margin: 10px;" method="POST" action="{{url('ctrl/nmcl/ingredient/'.$ingredient->id)}}">
        ...
        <div class="form-group {{$errors->first('name') ? 'has-danger' : ''}}">
            ...
            <input type="text" class="form-control {{$errors->first('name') ? 'form-control-danger' : ''}}" name="name"
                   id="att-name" value="{{$ingredient->name}}"
